	...
	if (m_Ninja.m_CurrentMoveTime > 0)
	{
		...
		// check if we Hit anything along the way
		{
			...
			for (int i = 0; i < Num; ++i)
			{
				...
				aEnts[i]->TakeDamage(vec2(0, -10.0f), m_Ninja.m_ActivationDir*-1, g_pData->m_Weapons.m_Ninja.m_pBase->m_Damage, m_pPlayer->GetCID(), WEAPON_NINJA, -1);
			}
		}
...
void CCharacter::FireWeapon()
{
	...
	bool FullAuto = false;
	if(m_ActiveWeapon == WEAPON_HAMMER || m_ActiveWeapon == WEAPON_GRENADE || m_ActiveWeapon == WEAPON_SHOTGUN || m_ActiveWeapon == WEAPON_LASER)
		FullAuto = true;
	...
	switch(m_ActiveWeapon)
	{
		case WEAPON_HAMMER:
		{
			...
			if (Num2 > 1 || m_CrateBuff == CRATE_BASEBALL)
			{
				for (int i = 0; i < Num2; ++i)
				{
					...
					pTarget->TakeDamage(vec2(0.f, -1.f) + normalize(Dir + vec2(0.f, -1.1f)) * 10.0f, Dir * -1, (m_CrateBuff == CRATE_BASEBALL ? 9 : g_pData->m_Weapons.m_Hammer.m_pBase->m_Damage),
						m_pPlayer->GetCID(), m_ActiveWeapon, -1);

					if(m_CrateBuff == CRATE_BASEBALL)
						pTarget->m_Core.m_Vel += normalize(Dir + vec2(0, -1.1f)) * 40.0f;
			...
			{
				if (apBuildings[0]->Team() == m_pPlayer->GetTeam())
				{
					if (apBuildings[0]->Health() < apBuildings[0]->MaxHealth())
					{
						apBuildings[0]->IncreaseHealth(2);
						GameServer()->CreateWrenchHit(m_Pos);
						m_NumObjectsHit = 1;
					}
				}
				else
				{
					apBuildings[0]->TakeDamage(vec2(0, 0), 1, m_pPlayer->GetCID(), -1, m_ActiveWeapon);
					GameServer()->CreateWrenchHit(m_Pos);
					m_NumObjectsHit = 1;
				}
			}
			else if (IsGrounded() && m_BuildingType >= 0)
			{
				if (m_BuildTick != -1 && m_BuildTick + Server()->TickSpeed() * Config()->m_SvBuildDelay > Server()->Tick())
					GameServer()->SendChatMessage(m_pPlayer->GetCID(), "Please wait before constructing another building");
				else
				{
					if (((CGameControllerNODES*)GameServer()->m_pController)->BuildBuilding(m_Pos, m_BuildingType, m_pPlayer->GetTeam(), m_pPlayer->GetCID(), false, m_CrateBuff == CRATE_CONSTRUCTION))
					{
						m_BuildingType = -1;
						m_BuildTick = Server()->Tick();
						GameServer()->CreateWrenchHit(m_Pos);
						m_NumObjectsHit = 1;
					}
				}
		...
		case WEAPON_GUN:
		{
			new CProjectile(GameWorld(), WEAPON_GUN,
				...
				g_pData->m_Weapons.m_Gun.m_pBase->m_Damage, false, 0, -1, WEAPON_GUN, m_Rampage);
		...
		case WEAPON_SHOTGUN:
		{
			...
			for(int i = -ShotSpread; i <= ShotSpread; ++i)
			{
				...
				new CProjectile(GameWorld(), WEAPON_SHOTGUN,
					...
					g_pData->m_Weapons.m_Shotgun.m_pBase->m_Damage, false, 0, -1, WEAPON_SHOTGUN, m_Rampage);
			}
		...
		case WEAPON_GRENADE:
		{
			new CProjectile(GameWorld(), WEAPON_GRENADE,
				...
				g_pData->m_Weapons.m_Grenade.m_pBase->m_Damage, true, 0, SOUND_GRENADE_EXPLODE, WEAPON_GRENADE, m_Rampage);
		...
		case WEAPON_LASER:
		{
			new CLaser(GameWorld(), m_Pos, Direction, GameServer()->Tuning()->m_LaserReach, m_pPlayer->GetCID(), m_Rampage);
...
{
	if (m_Core.m_Frozen)
		mem_zero(&m_Core.m_Input, sizeof(CNetObj_PlayerInput));
	else
	...
	if (m_Ill && Server()->Tick() % 150 == 0)
		TakeDamage(vec2(0, 0), vec2(0, 0), 1, (m_IllInfecter >= 0 ? m_IllInfecter : -3), WEAPON_ILL, -1);

	if (m_Ill && Server()->Tick() % 25 == 0)
	{
		for (int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if (pChr)
			{
				if (distance(m_Pos, pChr->m_Pos) < 75.0f)
				{
					if (!pChr->m_Ill)
					{
						pChr->m_Ill = true;
						pChr->m_IllInfecter = m_pPlayer->GetCID();
					}
				}
			}
		}
	...
	if (m_Input.m_Special)
	{
		if (m_pPlayer->m_Rage >= 90)
		{
			m_pPlayer->m_Rage = 0;
			m_Rampage = true;
			m_RampageTick = Server()->Tick();
			GameServer()->CreateEffect(m_Pos, EFFECT_RAMPAGE);
		}
		else if (m_pPlayer->m_Rage >= 70)
		{
			m_pPlayer->m_Rage = 0;
			m_Bomb = true;
			GameServer()->CreateEffect(m_Pos, EFFECT_BOMB);
			GameServer()->CreateSound(m_Pos, SOUND_BOMB_TICKING);
		}
		else if (m_pPlayer->m_Rage >= 40)
		{
			m_pPlayer->m_Rage = 0;
			m_Healing = 10;
			GameServer()->CreateEffect(m_Pos, EFFECT_HEALING);
			GameServer()->CreateSound(m_Pos, SOUND_PICKUP_HEALTH);
		}
	}

	if (Server()->Tick() % 50 == 0)
		m_pPlayer->m_Rage = max(0, m_pPlayer->m_Rage - 1);

	if (m_Healing > 0 && Server()->Tick() % 50 == 0)
	{
		if (IncreaseHealth(1))
			GameServer()->CreateSound(m_Pos, SOUND_PICKUP_HEALTH);
		else
	...
	if (m_Rampage)
	{
		if (Server()->Tick() > m_RampageTick + 10 * Server()->TickSpeed())
		{
			m_Rampage = false;
			m_RampageTick = -1;
		}
	...
	if (m_CrateBuff > 0 && Server()->Tick() > m_CrateBuffTick + 10 * Server()->TickSpeed())
	{
		m_CrateBuff = 0;
		if (m_Core.m_Frozen)
			m_Core.m_Frozen = false;
	}
}
...
void CCharacter::Die(int Killer, int Weapon)
{
	...
	// BOMB! ALLAHU AKBAR!
	if (Killer != m_pPlayer->GetCID() && m_Bomb)
	{
		m_Bomb = false;
		GameServer()->CreateExplosion(vec2(m_Pos.x - 32, m_Pos.y - 32), m_pPlayer->GetCID(), WEAPON_BOMB, 10);
		GameServer()->CreateExplosion(vec2(m_Pos.x + 32, m_Pos.y - 32), m_pPlayer->GetCID(), WEAPON_BOMB, 10);
		GameServer()->CreateExplosion(vec2(m_Pos.x, m_Pos.y + 32), m_pPlayer->GetCID(), WEAPON_BOMB, 10);
...
bool CCharacter::TakeDamage(vec2 Force, vec2 Source, int Dmg, int From, int Weapon, int PrevFrom)
{
	m_Core.m_Vel += Force;
	...
	{
		if (GameServer()->m_pController->IsFriendlyFire(m_pPlayer->GetCID(), PrevFrom))
			return false;
	}
	else if(From != PLAYER_TURRET_GUN && From != PLAYER_TURRET_SHOTGUN)
	{
		int Team = TEAM_RED;
	...
	if (From < 0 && PrevFrom >= 0) // turret. Choose turret owner as killer
		From = PrevFrom;
...
void CCharacter::Snap(int SnappingClient)
{
	...
	pCharacter->m_Rage = m_pPlayer->m_Rage;
	pCharacter->m_RampageTick = m_RampageTick;
	pCharacter->m_Effect = 0;
	if (m_Healing > 0)
		pCharacter->m_Effect |= EFFECT_HEALING;
	if (m_Rampage)
		pCharacter->m_Effect |= EFFECT_RAMPAGE;
	if (m_Bomb)
		pCharacter->m_Effect |= EFFECT_BOMB;

	pCharacter->m_CrateBuff = m_CrateBuff;
	pCharacter->m_CrateBuffMisc = m_CrateBuffMisc;
	pCharacter->m_CrateBuffTick = m_CrateBuffTick;
...
void CCharacter::BuildingSelect(int Type)
{
	if (Type / 4 + 1 > ((CGameControllerNODES*)GameServer()->m_pController)->m_aTechLevel[m_pPlayer->GetTeam()])
	{
		GameServer()->SendChatMessage(m_pPlayer->GetCID(), "Your team hasn't reached the tech level for this building yet");
		return;
	}
